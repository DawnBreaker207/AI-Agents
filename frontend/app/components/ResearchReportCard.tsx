    ...
    return (
        <div className="space-y-6">
            ...
            <Card className="border-l-4 border-l-primary shadow-sm">
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-4 text-sm text-muted-foreground pt-1">
                                <span className="flex items-center gap-1">
                                    <MapPin size={14}/> {report.regions?.join(", ") || "N/A"}</span>
                                <span
                ...
                <CardContent>
                    <p className="text-slate-700 leading-relaxed italic border-l-2 pl-4">"{report.summary || report.executive_summary}"</p>
                </CardContent>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2 text-sm">
                        <p><strong>Thị trường:</strong> {report.employment_status?.market || "N/A"}</p>
                        <p><strong>Nhu cầu:</strong> {report.employment_status?.demand || "N/A"}</p>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2 text-sm">
                        <p><strong>Lương:</strong> {report.job_details?.salary || "N/A"}</p>
            ...
            <div className="pt-4 border-t text-[10px] text-muted-foreground flex justify-between">
                <div className="flex gap-2">
                    Nguồn trích dẫn: {report.sources?.map((s, i) => <span key={i}
                                                                         className="hover:underline cursor-pointer">[{i + 1}]</span>) || "N/A"}
                </div>
                <div>Cập nhật gần nhất: {report.last_updated || "N/A"}</div>
            </div>
        </div>
    );
}
